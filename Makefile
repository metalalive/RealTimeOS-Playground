...
TARGET_IT = $(TARGET)-itest
TARGET_UT = $(TARGET)-utest
...
C_SOURCES = $(OS_C_SOURCES) $(HW_C_SOURCES) $(APPCFG_C_SOURCES)

# ASM sources
HW_ASM_SOURCES = \
Src/tests/hardware/stm32f4xx/startup_stm32f446xx.s

ASM_SOURCES = $(HW_ASM_SOURCES)
...
C_INCLUDES = -IInc $(HW_C_INCLUDES)  $(OS_C_INCLUDES) \
...
C_INCLUDES += $(UTEST_C_INCLUDES) $(ITEST_C_INCLUDES)
# ifeq ($(UNIT_TEST), yes)
#     C_DEFS += -DUNIT_TEST
# endif
...
all: \
	$(BUILD_DIR)/$(TARGET_IT).elf  $(BUILD_DIR)/$(TARGET_IT).hex \
	$(BUILD_DIR)/$(TARGET_IT).text $(BUILD_DIR)/$(TARGET_IT).bin
...
OBJS4UTEST = $(addprefix $(BUILD_DIR)/, $(UTEST_C_SOURCES:.c=.o))
vpath %.c $(sort $(dir $(UTEST_C_SOURCES)))

OBJS4ITEST = $(addprefix $(BUILD_DIR)/, $(ITEST_C_SOURCES:.c=.o))
vpath %.c $(sort $(dir $(ITEST_C_SOURCES)))
...
$(BUILD_DIR)/$(TARGET_IT).elf: $(OBJS4LIB) $(OBJS4ITEST)
	$(CC) $(LDFLAGS) $^ -o $@
	$(SZ) $@

$(BUILD_DIR)/$(TARGET_UT).elf: $(OBJS4LIB) $(OBJS4UTEST)
	$(CC) $(LDFLAGS) $^ -o $@
	$(SZ) $@
