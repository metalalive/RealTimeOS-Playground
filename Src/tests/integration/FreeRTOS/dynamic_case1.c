...
static void vDynPrControlTasksHandler(void *pvParams) {
    int32_t expectedVal = 0, actualVal = 0;
...
void vStartDynamicPriorityCase1( UBaseType_t uxPriority )
{
    ...
    const TaskFunction_t taskFuncs[NUM_OF_TASKS] = {
        vDynPrContinuousIncrementHandler, vDynPrBatchIncrementHandler, vDynPrControlTasksHandler,
    };
    ...
    for(idx=0; idx<NUM_OF_TASKS; idx++) {
        TaskParameters_t tskparams = {
            taskFuncs[idx], tasknames[idx], intgSTACK_SIZE, (void *) NULL,
            taskPriority[idx], stackMemSpace[idx], 
        };
